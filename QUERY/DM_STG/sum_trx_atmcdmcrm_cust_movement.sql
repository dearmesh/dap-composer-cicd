WITH cte_current_month AS (
  SELECT 
       ...
       ,Last_trx_date  
   FROM (
         SELECT atmcdmcrm.cif AS CIF
               ,gccf.level2 trx_type_level_2
               ,atmcdmcrm.channel_type
               ,SUM(atmcdmcrm.no_of_trx) not_feature
               ,SUM(atmcdmcrm.amount) amt_feature
               ,MIN(atmcdmcrm.trx_date) date_new_to_feature
               ,MAX(atmcdmcrm.trx_date) Last_trx_date 
           FROM (
                 SELECT cif
                       ,trx_type
                       ,channel_type
                       ...
                       ,amount
                       ,trx_date 
                   FROM dm_dap.dm_channel_trx_cdm_new
                  WHERE 1=1
                 --   AND date_pr BETWEEN FORMAT_DATE('%Y%m01', CAST(\'''' || cursor.PROCESS_BUSINESS_DATE || '''\' AS DATE)) AND FORMAT_DATE('%Y%m%d', CAST(\'''' || cursor.PROCESS_BUSINESS_DATE || '''\' AS DATE)) --ambil data 1 bulan
                 UNION ALL
                 SELECT cif_no AS cif
                       ,trx_type
                       ,channel AS channel_type
                       ,no_of_trx
                       ,amount
                       ,trx_date 
                   FROM dm_dap.dm_channel_trx_atm_off_us_new
                  WHERE 1=1
                 --   AND date_pr BETWEEN FORMAT_DATE('%Y%m01', CAST(\'''' || cursor.PROCESS_BUSINESS_DATE || '''\' AS DATE)) AND FORMAT_DATE('%Y%m%d', CAST(\'''' || cursor.PROCESS_BUSINESS_DATE || '''\' AS DATE)) --ambil data 1 bulan
                 UNION ALL
                 SELECT cif 
         ...
         LEFT JOIN dm_dap.grouping_channel_cleansing_fbi  gccf ON UPPER(TRIM(atmcdmcrm.trx_type)) = UPPER(TRIM(gccf.trx_type))
          WHERE 1=1
            AND gccf.fin_type = 'Fin'
         GROUP BY atmcdmcrm.cif, gccf.level2, atmcdmcrm.channel_type
        ) dctdn
   LEFT JOIN (
              SELECT * 
...
--------------- FINANCIAL SUM TOTAL PER CIF ---------------
  ...
  SELECT cif_funding cif
        ...
        ,date_new_to_feature
        ,Last_trx_date 
    FROM dm_stg_dap.sum_trx_dbankpro_cust_movement dctdn
   WHERE 1=1
