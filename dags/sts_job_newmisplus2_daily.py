...
from DATA.operator.dap.function_sts_job_newmisplus2_daily import Function
...
AIRFLOW_VAR_DAP_DAG_ID = "sts_job_newmisplus2_daily"
AIRFLOW_VAR_DAP_DAG_DESCRIPTION = "Cloudera NEWMISPLUS2 to BQ MISPLUS Daily"
AIRFLOW_VAR_DAP_DAG_ALIAS = 'sts_job_newmisplus2_daily'
AIRFLOW_VAR_DAP_DAG_PATH = '/DATA/config/dap/sts_job_newmisplus2_daily/'
...
with DAG(AIRFLOW_VAR_DAP_DAG_ID
    ...
    , catchup=False
    , tags=["STS","DAILY","NEWMISPLUS2","DAP"]) as AIRFLOW_VAR_DAP_DAG_ALIAS:
    ...
    for config_name, xfer_config in yaml_configs:
                ...
                configuration={
                    "query": {
                        "query": "CALL `"+AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID+ "`." + AIRFLOW_VAR_DAP_DATASET_SP + "." + AIRFLOW_VAR_DAP_SP_VALIDATE_MASTER_JOB +"\
                                        ...
                                        '"+AIRFLOW_VAR_DAP_DAG_ID+"'\
                                        ,'"+AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID+"'\
                                        ,'"+AIRFLOW_VAR_DAP_DATASET_MISPLUS+"'\
        ...
        get_variable_omt_master_job = PythonOperator(
            ...
            , op_kwargs={'query_get_var_{}'.format(config_name.replace("_yaml", "")): " SELECT \
                                                                                        ...
                                                                                        WHERE 1=1\
                                                                                            ...
                                                                                            and TABLE_NAME = '" + AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID + "." + AIRFLOW_VAR_DAP_DATASET_MISPLUS + "." + xfer_config.main_bigquery_table.table_name + "' \
                                                                                        "
                                    }
            , retries = 5
        )
            ...
            , op_kwargs={"initial_table_list_{}".format(config_name.replace("_yaml", "")) : "{{ dag_run.conf.get('initial_table_list','incremental') }}",
                        ...
                        "query_omt_{}".format(config_name.replace("_yaml", "")):"SELECT case when count(1) >= 1 then 1 else 0 end as COUNT_OMT \
                                                                                            ...
                                                                                            WHERE 1=1  \
                                                                                                ...
                                                                                                AND JOB_NAME = '" + AIRFLOW_VAR_DAP_DAG_ID + "' \
                                                                                                AND TRG_TBL_NAME = '" + AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID + "." + AIRFLOW_VAR_DAP_DATASET_MISPLUS + ".{{ task_instance.xcom_pull(task_ids='get_variable_omt_master_job_" + config_name.replace("_yaml", "") + "',key='target_table_name_" + config_name.replace("_yaml", "") + "') }}' \
                                                                                                AND EXECUTION_DATE <= '{{ execution_date.in_timezone('Asia/Jakarta').strftime('%Y-%m-%d') }}' \
        ...
        validate_enable_flag_and_duplicate_run = PythonOperator(
            ...
            , op_kwargs={'query_validate_{}'.format(config_name.replace("_yaml", "")): " SELECT \
                                                                                        ( SELECT case when count(1) >= 1 then 1 else 0 end as COUNT_MAIN_PROCESS \
                                                                                            ...
                                                                                            WHERE 1=1  \
                                                                                                AND JOB_NAME = '" + AIRFLOW_VAR_DAP_DAG_ID + "' \
                                                                                                AND TABLE_NAME = '" + AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID + "." + AIRFLOW_VAR_DAP_DATASET_MISPLUS + ".{{ task_instance.xcom_pull(task_ids='get_variable_omt_master_job_" + config_name.replace("_yaml", "") + "',key='target_table_name_" + config_name.replace("_yaml", "") + "') }}' \
        ...
        insert_omt_job_running = PythonOperator(
           ...
           ,op_kwargs={'query_{}'.format(config_name.replace("_yaml", "")): " INSERT INTO `" + AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID + "`." + AIRFLOW_VAR_DAP_OMT_PROCESS_LOG + "  \
                                                                                  VALUES ('" + AIRFLOW_VAR_DAP_JOB_TYPE + "'  \
                                                                                          ...
                                                                                          , 'N/A'  \
                                                                                          , '" + AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID + "." + AIRFLOW_VAR_DAP_DATASET_MISPLUS + ".{{ task_instance.xcom_pull(task_ids='get_variable_omt_master_job_" + config_name.replace("_yaml", "") + "',key='target_table_name_" + config_name.replace("_yaml", "") + "') }}'   \
                                                                                          , 0   \
                                                                                          , 0  \
                                                                                          , 0   \
                                                                                          , 'RUNNING'   \
        ...
        create_table_target = \
            BigQueryInsertJobOperator(
                ...
                configuration={
                    "query": {
                        "query": xfer_config.main_bigquery_table.schema_fields.replace('v_dataset_id',AIRFLOW_VAR_DAP_DATASET_MISPLUS).replace('v_connection',AIRFLOW_VAR_DAP_CONNECTION_BIGLAKE).replace('v_bucket',AIRFLOW_VAR_DAP_LANDING_BUCKET+"/{{ task_instance.xcom_pull(task_ids='get_variable_omt_master_job_" + config_name.replace("_yaml", "") + "',key='source_gcs_folder_" + config_name.replace("_yaml", "") + "') }}"),
                        "useLegacySql": False,
                    }
                },
                location= AIRFLOW_VAR_DAP_LOCATION,
