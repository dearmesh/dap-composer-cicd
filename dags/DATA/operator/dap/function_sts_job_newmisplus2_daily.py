...
class Function():
    ...
    def branch_validate_enable_flag_and_duplicate_run(self,**kwargs): 
        ...
        # # print("hahahahha wait_file_in_gcs_{}".format( "_".join(kwargs['task_instance'].task_id.split("_")[1:])))
        # ti = get_task_instance(self.AIRFLOW_VAR_DAP_DAG_ID, "validate_duplicate_run_{}".format( "_".join(kwargs['task_instance'].task_id.split("_")[1:])), exec_date)
        # state = ti.current_state()
        # print(state)
        # print(ti)

        # if state == 'success':
        #     return 'create_table_target_{}'.format( "_".join(kwargs['task_instance'].task_id.split("_")[1:]))
        # else:
        #     return 'Finish'
