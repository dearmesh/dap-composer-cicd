...
from DATA.operator.dap.function_dw_dap_fact_monthly import Function
...
AIRFLOW_VAR_DAP_JOB_TYPE = 'dw_dap_monthly'
AIRFLOW_VAR_DAP_DAG_ID = "dw_dap_fact_monthly"
AIRFLOW_VAR_DAP_DAG_DESCRIPTION = "Job for FACT DAP Monthly"
AIRFLOW_VAR_DAP_DAG_ALIAS = 'dw_dap_fact_monthly'
AIRFLOW_VAR_DAP_DAG_PATH = '/DATA/config/dap/dw_dap_fact_monthly/'
AIRFLOW_VAR_DAP_CATEGORY_JOB = 'DW'
...
with DAG(AIRFLOW_VAR_DAP_DAG_ID
    ...
    , catchup=False
    , tags=["DW","MONTHLY","FACT","DAP"]) as AIRFLOW_VAR_DAP_DAG_ALIAS:
    ...
    for config_name, xfer_config in yaml_configs:
        ...
        get_variable_omt_master_job = PythonOperator(
            ...
            , op_kwargs={'query_get_var_{}'.format(config_name.replace("_yaml", "")): " SELECT \
                                                                                        ...
                                                                                        WHERE 1=1\
                                                                                            ...
                                                                                            and TABLE_NAME = '" + AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID + "." + AIRFLOW_VAR_DAP_DATASET_DW + "." + xfer_config.main_bigquery_table.table_name + "' \
                                                                                        "
                                    }
            , retries = 5
        )
        ...
        validate_enable_flag_and_duplicate_run = PythonOperator(
            ...
            , op_kwargs={'query_validate_{}'.format(config_name.replace("_yaml", "")): " SELECT \
                                                                                        ( SELECT case when count(1) >= 1 then 1 else 0 end as COUNT_MAIN_PROCESS \
                                                                                            ...
                                                                                            WHERE 1=1  \
                                                                                                AND JOB_NAME = '" + AIRFLOW_VAR_DAP_DAG_ID + "' \
                                                                                                AND TABLE_NAME = '" + AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID + "." + AIRFLOW_VAR_DAP_DATASET_DW + ".{{ task_instance.xcom_pull(task_ids='get_variable_omt_master_job_" + config_name.replace("_yaml", "") + "',key='target_table_name_" + config_name.replace("_yaml", "") + "') }}' \
        ...
        create_table_target = \
            BigQueryInsertJobOperator(
                ...
                configuration={
                    "query": {
                        "query": xfer_config.main_bigquery_table.schema_fields.replace('v_dataset_id',AIRFLOW_VAR_DAP_DATASET_DW),
                        "useLegacySql": False,
                    }
                },
                location= AIRFLOW_VAR_DAP_LOCATION,
                                    ...
                                    "query": "UPDATE `" + AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID + "`." + AIRFLOW_VAR_DAP_OMT_PROCESS_LOG + " \
                                              SET STATUS = 'ERROR' \
                                                WHERE 1=1  \
                                                    ...
                                                    AND JOB_NAME = '" + AIRFLOW_VAR_DAP_DAG_ID + "' \
                                                    AND TRG_TBL_NAME = '" + AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID + "." + AIRFLOW_VAR_DAP_DATASET_DW + ".{{ task_instance.xcom_pull(task_ids='get_variable_omt_master_job_" + config_name.replace("_yaml", "") + "',key='target_table_name_" + config_name.replace("_yaml", "") + "') }}' \
        ...
        insert_omt_job_running = PythonOperator(
           ...
           ,op_kwargs={'query_{}'.format(config_name.replace("_yaml", "")): " INSERT INTO `" + AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID + "`." + AIRFLOW_VAR_DAP_OMT_PROCESS_LOG + "  \
                                                                                  VALUES ('" + AIRFLOW_VAR_DAP_JOB_TYPE + "'  \
                                                                                          ...
                                                                                          , 'DATAMART_QUERY'  \
                                                                                          , 'N/A'  \
                                                                                          , '" + AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID + "." + AIRFLOW_VAR_DAP_DATASET_DW + ".{{ task_instance.xcom_pull(task_ids='get_variable_omt_master_job_" + config_name.replace("_yaml", "") + "',key='target_table_name_" + config_name.replace("_yaml", "") + "') }}'   \
                                                                                          , 0   \
                                                                                          , 0  \
                                                                                          , 0   \
                                                                                          , 'RUNNING'   \
                            ...
                            , 'insert_omt_backlog_with_process_{}'.format(config_name.replace("_yaml","")) : "INSERT INTO `"+ AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID +"`."+ AIRFLOW_VAR_DAP_OMT_PROCESS_LOG +"  \
                                                                                                                ...
                                                                                                                ( \
                                                                                                                    SELECT \
                                                                                                                            ...
                                                                                                                            , 'N/A'  TRG_TEMP_FILE\
                                                                                                                            , '"+ AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID + "." + AIRFLOW_VAR_DAP_DATASET_DW +".{{ task_instance.xcom_pull(task_ids='get_variable_omt_master_job_" + config_name.replace("_yaml", "") + "',key='target_table_name_" + config_name.replace("_yaml", "") + "') }}'  TRG_TBL_NAME \
                                                                                                                            , 0  DELTA_DATA_COUNT \
                                                                                                                            , 0   SRC_DATA_COUNT\
                                                                                                                            , NULL   TRG_DATA_COUNT\
                                                                                                                            , 'RUNNING'  STATUS  \
        ...
        #                         "query": {
        #                                     "query": "DELETE \
        #                                             FROM `" + AIRFLOW_VAR_DAP_SANDBOX_PROJECT_ID + "`." + AIRFLOW_VAR_DAP_DATASET_DW + ".{{ task_instance.xcom_pull(task_ids='get_variable_omt_master_job_" + config_name.replace("_yaml", "") + "',key='target_table_name_" + config_name.replace("_yaml", "") + "') }} \
        #                                             WHERE 1=1 \
        ...
        #                         "query": {
        #                                     "query": "SELECT * \
        #                                             FROM `" + AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID + "`." + AIRFLOW_VAR_DAP_DATASET_DW + ".{{ task_instance.xcom_pull(task_ids='get_variable_omt_master_job_" + config_name.replace("_yaml", "") + "',key='target_table_name_" + config_name.replace("_yaml", "") + "') }} \
        #                                             WHERE 1=1 \
        #                                                 AND BUSINESS_DATE = LAST_DAY(DATE_SUB(CAST('{{ execution_date.in_timezone('Asia/Jakarta').strftime('%Y-%m-%d') }}' AS DATE), INTERVAL 1 MONTH)) \
        #                                                 "
        #                                     , "useLegacySql": False
        ...
        #                                                             , "datasetId": AIRFLOW_VAR_DAP_DATASET_DW
        ...
        create_table_for_housekeeping = BigQueryOperator(
            ...
            , sql="SELECT * \
                ...
                ORDER BY 1"
            , destination_dataset_table= AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID + "." + AIRFLOW_VAR_DAP_DATASET_TEMP_CLEANSING + "." + AIRFLOW_VAR_DAP_DATASET_DW + "_{{ task_instance.xcom_pull(task_ids='get_variable_omt_master_job_" + config_name.replace("_yaml", "") + "',key='target_table_name_" + config_name.replace("_yaml", "") + "') }}_clean"  
