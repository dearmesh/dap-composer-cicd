...
AIRFLOW_VAR_DAP_DATASET_DM_STG_MASKED = Variable.get('AIRFLOW_VAR_DAP_DATASET_DM_STG_MASKED')
AIRFLOW_VAR_DAP_DATASET_DW = Variable.get('AIRFLOW_VAR_DAP_DATASET_DW')
AIRFLOW_VAR_DAP_DATASET_DW_MASKED = Variable.get('AIRFLOW_VAR_DAP_DATASET_DW_MASKED')
...
def omt_table():
    ...
    bigquery_hook = BigQueryHook(gcp_conn_id='google_cloud_default', delegate_to=None, use_legacy_sql=False,location=AIRFLOW_VAR_DAP_LOCATION)
    ...
    for status in list_status:
        ...
        row_data = bigquery_hook.get_records(sql="SELECT JOB_TYPE,JOB_NAME,START_DATE,END_DATE,SRC_FILE_OR_TBL_NAME,TRG_TBL_NAME ,STATUS, SPLIT(SUBSTRING(CAST(END_DATE  - START_DATE AS STRING),6),'.')[OFFSET(0)] DURATION from OMT.OMT_PROCESS_LOG WHERE STATUS='"+status+"' AND EXECUTION_DATE='"+Variable.get('AIRFLOW_VAR_DAP_CURRENT_EXECUTION_DATE')+"' ORDER BY JOB_TYPE,JOB_NAME,START_DATE,TRG_TBL_NAME")
...
def set_date(**kwargs):
    Variable.set(key="AIRFLOW_VAR_DAP_CURRENT_EXECUTION_DATE", value=kwargs['execution_date'])  
...
with DAG(
    ...
    send_email = EmailOperator( 
                            ...
                            to=AIRFLOW_VAR_DAP_EMAIL_SEND_TO, 
