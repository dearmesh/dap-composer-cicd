...
from DATA.operator.dap.function_dm_stg_dap_monthly import Function
...
AIRFLOW_VAR_DAP_JOB_TYPE = 'dm_stg_dap_monthly'
AIRFLOW_VAR_DAP_DAG_ID = "dm_stg_dap_monthly"
AIRFLOW_VAR_DAP_DAG_DESCRIPTION = "Job for DM STG DAP Monthly"
AIRFLOW_VAR_DAP_DAG_ALIAS = 'dm_stg_dap_monthly'
AIRFLOW_VAR_DAP_DAG_PATH = '/DATA/config/dap/dm_stg_dap_monthly/'
AIRFLOW_VAR_DAP_CATEGORY_JOB = 'DM_STG'
    ...
    for config_name, xfer_config in yaml_configs:
                ...
                configuration={
                    "query": {
                        "query": "CALL `"+AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID+ "`." + AIRFLOW_VAR_DAP_DATASET_SP + "." + AIRFLOW_VAR_DAP_SP_VALIDATE_MASTER_JOB +"\
                                        ...
                                        '"+AIRFLOW_VAR_DAP_DAG_ID+"'\
                                        ,'"+AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID+"'\
                                        ,'"+AIRFLOW_VAR_DAP_DATASET_DM_STG+"'\
        ...
        get_variable_omt_master_job = PythonOperator(
            ...
            , op_kwargs={'query_get_var_{}'.format(config_name.replace("_yaml", "")): " SELECT \
                                                                                        ...
                                                                                        WHERE 1=1\
                                                                                            ...
                                                                                            and TABLE_NAME = '" + AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID + "." + AIRFLOW_VAR_DAP_DATASET_DM_STG + "." + xfer_config.main_bigquery_table.table_name + "' \
                                                                                        "
                                    }
            , retries = 5
        )
        ...
        validate_enable_flag_and_duplicate_run = PythonOperator(
            ...
            , op_kwargs={'query_validate_{}'.format(config_name.replace("_yaml", "")): " SELECT \
                                                                                        ( SELECT case when count(1) >= 1 then 1 else 0 end as COUNT_MAIN_PROCESS \
                                                                                            ...
                                                                                            WHERE 1=1  \
                                                                                                AND JOB_NAME = '" + AIRFLOW_VAR_DAP_DAG_ID + "' \
                                                                                                AND TABLE_NAME = '" + AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID + "." + AIRFLOW_VAR_DAP_DATASET_DM_STG + ".{{ task_instance.xcom_pull(task_ids='get_variable_omt_master_job_" + config_name.replace("_yaml", "") + "',key='target_table_name_" + config_name.replace("_yaml", "") + "') }}' \
        ...
        create_table_target = \
            BigQueryInsertJobOperator(
                ...
                configuration={
                    "query": {
                        "query": xfer_config.main_bigquery_table.schema_fields.replace('v_dataset_id',AIRFLOW_VAR_DAP_DATASET_DM_STG),
                        "useLegacySql": False,
                    }
                },
                location= AIRFLOW_VAR_DAP_LOCATION,
                                    ...
                                    "query": "UPDATE `" + AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID + "`." + AIRFLOW_VAR_DAP_OMT_PROCESS_LOG + " \
                                              SET STATUS = 'ERROR' \
                                                WHERE 1=1  \
                                                    ...
                                                    AND JOB_NAME = '" + AIRFLOW_VAR_DAP_DAG_ID + "' \
                                                    AND TRG_TBL_NAME = '" + AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID + "." + AIRFLOW_VAR_DAP_DATASET_DM_STG + ".{{ task_instance.xcom_pull(task_ids='get_variable_omt_master_job_" + config_name.replace("_yaml", "") + "',key='target_table_name_" + config_name.replace("_yaml", "") + "') }}' \
        ...
        insert_omt_job_running = PythonOperator(
           ...
           ,op_kwargs={'query_{}'.format(config_name.replace("_yaml", "")): " INSERT INTO `" + AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID + "`." + AIRFLOW_VAR_DAP_OMT_PROCESS_LOG + "  \
                                                                                  VALUES ('" + AIRFLOW_VAR_DAP_JOB_TYPE + "'  \
                                                                                          ...
                                                                                          , 'DATAMART_QUERY'  \
                                                                                          , 'N/A'  \
                                                                                          , '" + AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID + "." + AIRFLOW_VAR_DAP_DATASET_DM_STG + ".{{ task_instance.xcom_pull(task_ids='get_variable_omt_master_job_" + config_name.replace("_yaml", "") + "',key='target_table_name_" + config_name.replace("_yaml", "") + "') }}'   \
                                                                                          , 0   \
                                                                                          , 0  \
                                                                                          , 0   \
                                                                                          , 'RUNNING'   \
                            ...
                            , 'insert_omt_backlog_with_process_{}'.format(config_name.replace("_yaml","")) : "INSERT INTO `"+ AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID +"`."+ AIRFLOW_VAR_DAP_OMT_PROCESS_LOG +"  \
                                                                                                                ...
                                                                                                                ( \
                                                                                                                    SELECT \
                                                                                                                            ...
                                                                                                                            , 'N/A'  TRG_TEMP_FILE\
                                                                                                                            , '"+ AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID + "." + AIRFLOW_VAR_DAP_DATASET_DM_STG +".{{ task_instance.xcom_pull(task_ids='get_variable_omt_master_job_" + config_name.replace("_yaml", "") + "',key='target_table_name_" + config_name.replace("_yaml", "") + "') }}'  TRG_TBL_NAME \
                                                                                                                            , 0  DELTA_DATA_COUNT \
                                                                                                                            , 0   SRC_DATA_COUNT\
                                                                                                                            , NULL   TRG_DATA_COUNT\
                                                                                                                            , 'RUNNING'  STATUS  \
        ...
        #                         "query": {
        #                                     "query": "DELETE \
        #                                             FROM `" + AIRFLOW_VAR_DAP_SANDBOX_PROJECT_ID + "`." + AIRFLOW_VAR_DAP_DATASET_DM_STG + ".{{ task_instance.xcom_pull(task_ids='get_variable_omt_master_job_" + config_name.replace("_yaml", "") + "',key='target_table_name_" + config_name.replace("_yaml", "") + "') }} \
        #                                             WHERE 1=1 \
        ...
        #                         "query": {
        #                                     "query": "SELECT * \
        #                                             FROM `" + AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID + "`." + AIRFLOW_VAR_DAP_DATASET_DM_STG + ".{{ task_instance.xcom_pull(task_ids='get_variable_omt_master_job_" + config_name.replace("_yaml", "") + "',key='target_table_name_" + config_name.replace("_yaml", "") + "') }} \
        #                                             WHERE 1=1 \
        #                                                 AND BUSINESS_DATE = LAST_DAY(DATE_SUB(CAST('{{ execution_date.in_timezone('Asia/Jakarta').strftime('%Y-%m-%d') }}' AS DATE), INTERVAL 1 MONTH)) \
        #                                                 "
        #                                     , "useLegacySql": False
        ...
        #                                                             , "datasetId": AIRFLOW_VAR_DAP_DATASET_DM_STG
        ...
        create_table_for_housekeeping = BigQueryOperator(
            ...
            , sql="SELECT * \
                ...
                ORDER BY 1"
            , destination_dataset_table= AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID + "." + AIRFLOW_VAR_DAP_DATASET_TEMP_CLEANSING + "." + AIRFLOW_VAR_DAP_DATASET_DM_STG + "_{{ task_instance.xcom_pull(task_ids='get_variable_omt_master_job_" + config_name.replace("_yaml", "") + "',key='target_table_name_" + config_name.replace("_yaml", "") + "') }}_clean"  
        ...
        housekeeping_data_to_archive = BashOperator(
            ...
            , bash_command= "bq extract \
                            ...
                            --field_delimiter=',' " + AIRFLOW_VAR_DAP_PROCESSING_PROJECT_ID + ":" + AIRFLOW_VAR_DAP_DATASET_TEMP_CLEANSING + "." + AIRFLOW_VAR_DAP_DATASET_DM_STG + "_{{ task_instance.xcom_pull(task_ids='get_variable_omt_master_job_" + config_name.replace("_yaml", "") + "',key='target_table_name_" + config_name.replace("_yaml", "") + "') }}_clean gs://"+ AIRFLOW_VAR_DAP_ARCHIVED_BUCKET +"/{{ task_instance.xcom_pull(task_ids='get_variable_omt_master_job_" + config_name.replace("_yaml", "") + "',key='archive_gcs_folder_" + config_name.replace("_yaml", "") + "') }}/data/"+ AIRFLOW_VAR_DAP_DATASET_DM_STG +"_{{ task_instance.xcom_pull(task_ids='get_variable_omt_master_job_" + config_name.replace("_yaml", "") + "',key='target_table_name_" + config_name.replace("_yaml", "") + "') }}_clean_"+"{{ execution_date.in_timezone('Asia/Jakarta').strftime('%Y-%m-%d') }}.csv"
            , retries = 10
        )
        ...
        Start >> validate_master_job >> get_variable_omt_master_job >> validate_enable_flag_and_duplicate_run >> branch_validate >>  create_table_target >> update_if_error_exists >> delete_omt_error >> insert_omt_job_running >> autofill_backlog >> check_dependency_job_running >> check_dependency_datamart >> insert_omt_running >> process_data_to_datamart >> autofill_backlog_insert_only >> create_table_for_housekeeping >> housekeeping_data_to_archive >> delete_data_in_table_for_housekeeping >> update_omt_job_status_flag >> update_omt_job_master_status_flag >> Finish
