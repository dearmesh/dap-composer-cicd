...
from DATA.operator.dap.function_downstream_dap_sts_daily import Function
...
AIRFLOW_VAR_DAP_JOB_TYPE = 'downstream_dap_sts_daily'
AIRFLOW_VAR_DAP_DAG_ID = "downstream_dap_sts_daily"
AIRFLOW_VAR_DAP_DAG_DESCRIPTION = "Parent Job for Downstream STS Job DAP Daily"
AIRFLOW_VAR_DAP_DAG_ALIAS = 'downstream_dap_sts_daily'
AIRFLOW_VAR_DAP_DAG_PATH = '/DATA/config/dap/downstream_dap_sts_daily/'
...
with DAG(AIRFLOW_VAR_DAP_DAG_ID
    ...
    , catchup=False
    , tags=["DOWNSTREAM","STS","DAILY","SEQ_MAIN","DAP"]) as AIRFLOW_VAR_DAP_DAG_ALIAS:
    
    Start = EmptyOperator(task_id="Start")

    t1 = TriggerDagRunOperator(
        task_id='sts_job_newmisplus2_daily'
        , trigger_dag_id='sts_job_newmisplus2_daily'
    ...
    t2 = TriggerDagRunOperator(
        task_id='sts_job_dmu_workspace_daily'
        , trigger_dag_id='sts_job_dmu_workspace_daily'
        , execution_date = "{{ next_execution_date.in_timezone('Asia/Jakarta') }}'" 
        , wait_for_completion=True
        , reset_dag_run=True
        , retries = 2
    ...
    t3 = TriggerDagRunOperator(
        task_id='sts_job_bd_tableau_daily'
        , trigger_dag_id='sts_job_bd_tableau_daily'
        , execution_date = "{{ next_execution_date.in_timezone('Asia/Jakarta') }}'" 
        , wait_for_completion=True
        , reset_dag_run=True
        , retries = 2
